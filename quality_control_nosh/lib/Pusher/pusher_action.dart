  ...
  void _waitForResponse() {
    ...
    _port!.inputStream!.listen((Uint8List data) async {
      ...
      if (data.isNotEmpty) {
        ...
        } else if (response.trim() == '0' && waitingForResponse) {
          // Received response code '0' and still waiting for response
          // Trigger success action only if not stopped due to -101
          waitingForResponse = false;
          timeoutTimer.cancel(); // Cancel the timeout timer
