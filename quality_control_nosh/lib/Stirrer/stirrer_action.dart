...
class StirrerAction extends StatefulWidget {
  final String qrData;

  const StirrerAction({Key? key, required this.qrData}) : super(key: key);

  @override
  _StirrerActionState createState() => _StirrerActionState();
}

class _StirrerActionState extends State<StirrerAction> {
  ...
  void _waitForResponse() {
    ...
    Timer timeoutTimer = Timer(Duration(seconds: 66), () {
  ...
  void _stopPlaying() async {
    try {
      ...
      if (await Permission.storage.request().isGranted) {
        ...
        String fileName = '$qrCodeData-$timestamp-Fail-Stirrer.txt';
                ...
                TextButton(
                  onPressed: () {
                    ...
                    Navigator.pushAndRemoveUntil(
                      context,
                      MaterialPageRoute(builder: (context) => MyStirrer()),
  ...
  void _success() async {
    try {
      ...
      if (await Permission.storage.request().isGranted) {
        ...
        final timestamp =
            '${now.year}-${now.month}-${now.day}_${now.hour}-${now.minute}-${now.second}';
        String fileName = '$qrCodeData-$timestamp-Success-Stirrer.txt';
