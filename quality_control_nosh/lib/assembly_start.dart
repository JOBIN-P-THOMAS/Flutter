...
class AssemblyDetailPage extends StatefulWidget {
  final String assemblyName;

  AssemblyDetailPage(this.assemblyName);

  @override
  _AssemblyDetailPageState createState() => _AssemblyDetailPageState();
}

class _AssemblyDetailPageState extends State<AssemblyDetailPage> {
  UsbPort? port;
  String response = '';
  ...
  void _startButtonAction() async {
    ...
    final commands = [
      {'command': 'CMD A 001', 'expectedResponse': '0'},
      {'command': 'CMD B 001', 'expectedResponse': '0'},
      {'command': 'CMD C 001', 'expectedResponse': '0'},
      {'command': 'CMD D 001', 'expectedResponse': '0'},
      {'command': 'CMD E 001', 'expectedResponse': '0'},
      {'command': 'CMD F 001', 'expectedResponse': '0'},
    ];
        ...
        child: Padding(
          ...
          child: Column(
            children: [
              ...
              _buildCheckContainer('Motor Check', checkStatus[2]),
              _buildCheckContainer('Motor Encoder Check', checkStatus[3]),
              _buildCheckContainer('BLDC Zero Check', checkStatus[4]),
              _buildCheckContainer('BLDC check', checkStatus[5]),
            ],
          ),
        ),
      ),
